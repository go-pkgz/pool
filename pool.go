...
func (p *WorkerGroup[T]) Go(ctx context.Context) error {
	...
	p.ctx = metrics.Make(ctx)

	var egCtx context.Context
	p.eg, egCtx = errgroup.WithContext(ctx)

	worker := func(id int, inCh chan []T) func() error {
		return func() error {
			var lastErr error
			var totalErrs int

			wCtx := metrics.WithWorkerID(p.ctx, id)
			...
			for {
				...
				case vv, ok := <-inCh:
					if !ok { // input channel closed
						if e := p.finalizeWorker(wCtx, id, worker); e != nil {
							return e
						}
						if lastErr != nil {
...
// Metrics returns a metrics for the pool's context
func (p *WorkerGroup[T]) Metrics() *metrics.Value {
	return metrics.Get(p.ctx)
}
